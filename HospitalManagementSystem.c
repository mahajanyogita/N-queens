...
void WelcomeScreen(void)
{
	...
	printf("\n\n\n\n\n\n\n\t\t\t\t##################################################");
	printf("\n\t\t\t\t#\t\t WELCOME TO\t\t         #");
	printf("\n\t\t\t\t#   SHREE SAMARTHA HOSPITAL MANAGEMENT SYSTEM    #");
	printf("\n\t\t\t\t##################################################");
...
char Username[15];
char Password[15];
char original_Username[25]="SHREESAMARTHA80";
char original_Password[15]="YOGITA";
...
while(e<=2);
	if(e>2)
	{
	printf("You have cross the limit. You cannot login.SORRY!!!!");
	getch();
    ex_it();
	}
...
do
{
	...
	{
		printf("\n\t Sorry :( Invalid.--> Contact no. must contain 10 numbers.\n Enter again :");
		goto D;
	}
	else
	{
...
void Edit_rec(void)
{
  ...
  system("cls");
  	Title();// call Title window
	   ...
	   fclose(ft);
	   fclose(ek);
	   remove("samartha1.dat");
   	   rename("yogita.dat1","samartha1.dat");
		getch();
		MainMenu();
}
void Dlt_rec()
{
	...
	else
	{
		...
		fclose(ft);
		remove("samartha1.dat");
		rename("namrata1.dat","samartha1.dat");
